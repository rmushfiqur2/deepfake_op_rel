...
train_set1 = "xception_features_real_"
train_set2 = "xception_features_fake_"
test_set1 = "xception_features_real_"
test_set2 = "xception_features_swapped_"  # features_swapped_ features_fake_
...
if __name__ == '__main__':
    ...
    for R in range(4):  # repeat process
        ...
        tmp_index = list(all_ids.difference(valid_index))  # total videos 5
        train_index = tmp_index
        ...
        for n in range(62):
            ...
            file = glob.glob(
                '../required-feature-folder-xception//saved_features/xception_features_real_'
            ...
            for idx in train_index:
                file = glob.glob('../required-feature-folder-xception//saved_features/' +
                                 train_set1 + name + '_v_' + str(idx) +
                                 '_recon_' + recons + '*')[0]
                with open(file, 'rb') as f:
                    res = pickle.load(f)
                real_videos_train = real_videos_train + [
                    sample_frames_N(res, frames_per_video)
                ]
            for idx in train_index2:
                file = glob.glob('../required-feature-folder-xception//saved_features/' +
                                 train_set2 + name + '_v_' + str(idx) +
                                 '_recon_' + recons + '*')[0]
                with open(file, 'rb') as f:
                    res = pickle.load(f)
        ...
        net_tuned, train_loss_history, valid_loss_history = Tune_NN_baseline_xception(
            device, initial_lr, epoch_num, real_videos_train,
            real_videos_valid, real_videos_test, fake_videos_train,
            fake_videos_valid, fake_videos_test, in_features, R)
        ...
        plot_AUC_names = [
            "Ryan Reynolds", "Will Ferrell", "Chris Pine", "Ben Affleck",
            "Kate Beckinsale", "Carrie-Anne Moss"
        ]
        plot_auc_serial = 0
        for person_id in valid_ids:
            ...
            if person_id in valid_ids_to_plot_auc:
                ...
                axes_auc[row_num, col_num].plot(false_pos_rate,
                                                true_pos_rate,
                                                color=R_colors[R])
                if col_num == 0:
                ...
                if R == 0:
                    axes_auc[row_num, col_num].text(0.5,
                                                    0.5,
                                                    'Sample Text',
                                                    fontsize=12,
                                                    ha='center')
