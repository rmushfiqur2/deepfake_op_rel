...
### backbone is trained as fake = 1, real = 0. so we will swap the labels for baseline tuning
exclude_ids = [42, 43, 44, 45, 19]
if __name__ == '__main__':
    ...
    for R in range(4): # repeat process
        ...
        for n in range(62):
            ...
            file = glob.glob('../required-feature-folder-efficientnet//saved_features/features_real_' + name + '_v_' + '*'+'_recon_'+recons+'*')
            if len(file) == 0:
                continue
            file = glob.glob('../required-feature-folder-efficientnet//saved_features/features_real_' + name_cacd + '_v_*' +'_recon_'+recons+'*')
            if len(file) == 0:
                continue
            ...
            file = glob.glob('../required-feature-folder-efficientnet//saved_features/'+test_set1+name_cacd+'_v_'+str(0)+'_recon_'+recons+'*')[0]
            with open(file, 'rb') as f: res = pickle.load(f)
            real_videos_test = real_videos_test + [sample_frames_N(res, frames_per_video)]
            test_id_cal_real.append(len(real_videos_test))

            file = glob.glob('../required-feature-folder-efficientnet//saved_features/'+test_set2+name_cacd+'_v_'+str(0)+'_recon_'+recons+'*')[0]
            with open(file, 'rb') as f: res = pickle.load(f)
            fake_videos_test = fake_videos_test + [sample_frames_N(res, frames_per_video)]
            test_id_cal_fake.append(len(fake_videos_test))

        in_features = real_videos_test[0][0].shape[1]
        net_tuned = get_Tuned_NN_baseline_efficientnet(device, in_features, R)

        net_tuned.eval()
        jj = 0
        valid_ids_to_plot_auc = [3, 6, 16, 27, 47, 57]
        plot_AUC_names = ["Ryan Reynolds", "Will Ferrell", "Chris Pine", "Ben Affleck", "Kate Beckinsale", "Carrie-Anne Moss"]
        plot_auc_serial = 0
        for person_id in valid_ids:
            ...
            for i in range(test_id_cal_real[jj], test_id_cal_real[jj+1]):
                ...
                if len(out1) != len(out2):
                    ...
                    out1 = out1[:tmp]
                    out2 = out2[:tmp]
                out_feature = net_tuned(out1.to(device))
                res_real_frame = res_real_frame + list(out_feature.cpu().detach().numpy())
            for i in range(test_id_cal_fake[jj], test_id_cal_fake[jj+1]):
                ...
                if len(out1) != len(out2):
                    ...
                    out2 = out2[:tmp]
                out_feature = net_tuned(out1.to(device))
                res_fake_frame = res_fake_frame + list(out_feature.cpu().detach().numpy())

            Y_test =  [1] * len(res_fake_frame) + [0] * len(res_real_frame)
            ...
            if person_id in valid_ids_to_plot_auc:
                ...
                axes_auc[row_num, col_num].plot(false_pos_rate, true_pos_rate, color=R_colors[R])
                if col_num == 0:
                    axes_auc[row_num, col_num].set_ylabel('true positive rate')
                if row_num == 1:
                    axes_auc[row_num, col_num].set_xlabel('false positive rate')
                if R==0:
                    axes_auc[row_num, col_num].text(0.5, 0.5, 'Sample Text', fontsize=12, ha='center')
                    axes_auc[row_num, col_num].set_xlim(0.0, 1.0)
                    axes_auc[row_num, col_num].set_ylim(0.0, 1.0)
                    axes_auc[row_num, col_num].grid(True)
                    axes_auc[row_num, col_num].set_xticks([0.0, 0.25, 0.5, 0.75, 1.0],['0','.25','.5','.75','1'])
                    axes_auc[row_num, col_num].set_yticks([0.0, 0.25, 0.5, 0.75, 1.0],['0','.25','.5','.75','1'])
                plot_auc_serial += 1
                print('Cross dataset validation: vaidation celebdf'+'  AUC=', roc_auc)
