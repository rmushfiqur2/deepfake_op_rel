...
class DataLoader(object):
    def __init__(self, filenames, all_filenames, batch_size, dir_bm_eyes, 
                 resolution, num_cpus, sess, inference_mode, **da_config):
        ...
        if inference_mode:
            self.data_iter_next = self.create_tfdata_iter_inference(
                self.filenames,
                self.all_filenames,
                self.batch_size,
                self.dir_bm_eyes,
    ...
    def create_tfdata_iter(self, filenames, fns_all_trn_data, batch_size, dir_bm_eyes, resolution, 
        ...
        dataset = dataset.apply(
            tf.contrib.data.map_and_batch(
                ...
                drop_remainder=True # reaminder not drop tf 1.6.0 doesn't have drop_remainder
            )
        )
    ...
    def create_tfdata_iter_inference(self, filenames, fns_all_trn_data, batch_size, dir_bm_eyes, resolution,
                           prob_random_color_match, use_da_motion_blur, use_bm_eyes):
        tf_fns = tf.constant(filenames, dtype=tf.string) # use tf_fns=filenames is also fine
        dataset = tf.data.Dataset.from_tensor_slices(tf_fns)
        ...
        dataset = dataset.apply(
            tf.contrib.data.map_and_batch(
                ...
                batch_size=batch_size,
                num_parallel_batches=self.num_cpus,  # cpu cores
                drop_remainder=False # reaminder not drop tf 1.6.0 doesn't have drop_remainder
            )
        )
        dataset = dataset.repeat()
        dataset = dataset.prefetch(32)
