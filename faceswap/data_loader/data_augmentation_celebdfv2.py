...
def read_image(fn, dir_bm_eyes=None, res=64, use_bm_eyes=True):
    ...
    #warped_img, target_img = random_warp_rev(image, res=res) # both zoomed. zoomed -> affine -> warped_img -> reverse
    # affine -> target image
    
    #bm_eyes = target_img[...,3:]
    #target_img = target_img[...,:3]

    #target_img, bm_eyes = target_img.astype(np.float32), bm_eyes.astype(np.float32)
    return image.astype(np.float32), bm_eyes.astype(np.float32)