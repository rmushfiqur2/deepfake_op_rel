...
from .data_augmentation_celebdfv2 import *


class DataLoader(object):
    def __init__(self, filenames, batch_size, dir_bm_eyes,
                 resolution, num_cpus, sess, **da_config):
        # inference_mode added by mushfiq
    ...
    def set_data_augm_config(self, prob_random_color_match=0.5, use_da_motion_blur=True, use_bm_eyes=True):
    ...
    def create_tfdata_iter(self, filenames, batch_size, dir_bm_eyes, resolution,
        ...
        # dataset = dataset.shuffle(len(filenames)) no shuffle
        dataset = dataset.apply(
            tf.contrib.data.map_and_batch(
                ...
                drop_remainder=False  # remainder not drop
            )
        )
        ...
        next_element = iterator.get_next()  # this tensor can also be used as Input(tensor=next_element)
        return next_element
