...
def showG(test_A, test_B, path_A, path_B, batchSize, once=False, save=False, save_path=None, filename=None):
    figure_A = np.stack([
        test_A,
    ...
    if not once:
        ...
        figure = figure_A
        figure = figure.reshape((2,batchSize//2) + figure.shape[1:])
    figure = stack_images(figure)
    figure = np.clip((figure + 1) * 255 / 2, 0, 255).astype('uint8')
    if save:
        if filename is None:
            filename = 'recons.jpg'
        cv2.imwrite(os.path.join(save_path, filename), figure)
    else:
        figure = cv2.cvtColor(figure, cv2.COLOR_BGR2RGB)
        display(Image.fromarray(figure))
    
def showG_mask(test_A, test_B, path_A, path_B, batchSize, once=False, save=False, save_path=None):
    figure_A = np.stack([
        test_A,
    ...
    figure = np.clip((figure + 1) * 255 / 2, 0, 255).astype('uint8')
    if save:
        cv2.imwrite(os.path.join(save_path, 'mask.jpg'), figure)
    else:
        figure = cv2.cvtColor(figure, cv2.COLOR_BGR2RGB)
        display(Image.fromarray(figure))

def showG_eyes(test_A, test_B, bm_eyes_A, bm_eyes_B, batchSize, once=False, save=False, save_path=None):
    figure_A = np.stack([
        (test_A + 1)/2,
        bm_eyes_A,
