...
for idx in range(101, 103):
    ...
    img_dirA_bm_eyes = '../faces/binary_masks-celebdf/'+ str(idx) +'_eyes'
    if not os.path.exists(img_dirA):
        continue
    print("working on id "+str(idx))

    # Path to saved model weights
    models_dir = "./models-celebdf/gens_5_videos/id_"+str(idx)
    ...
    # Get filenames
    video_filter = "/*" #"/id" + str(idx) + "_000[5-9]*"
    ...
    if use_bm_eyes:
        assert len(glob.glob(img_dirA_bm_eyes+video_filter)), "No binary mask found in " + str(img_dirA_bm_eyes)
        assert len(glob.glob(img_dirA_bm_eyes+video_filter)) == len(train_A), \
    ...
    def reset_session(save_path):
        ...
        train_batchA = DataLoader(train_A, train_AnB, batchSize, img_dirA_bm_eyes,
                                  RESOLUTION, num_cpus, K.get_session(), False, **da_config)
    ...
    for gen_iterations in range(start_iter, TOTAL_ITERS):
        ...
        if gen_iterations % display_iters == 0:
            ...
            % (gen_iterations, errDA_sum/display_iters,
               errGA_sum/display_iters, time.time()-t0))
            print("----------")
            ...
            showG(tA, tA, model.path_A, model.path_A, batchSize, once=False, save=True, save_path=models_dir,
                  filename="final_result.jpg")
            print("Masks:")
            showG_mask(tA, tA, model.path_mask_A, model.path_mask_A, batchSize, once=False, save=True, save_path=models_dir)
            print("Reconstruction results:")
            showG(wA, wA, model.path_bgr_A, model.path_bgr_A, batchSize, once=False, save=True, save_path=models_dir,
