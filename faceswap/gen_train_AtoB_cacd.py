...
id_cacds = table.id_cacd

#print(table.common_name)
#print(b.values)

for idx in range(101,102):  # 31 - 2 = 29 pairs (0-35 done)

    if idx<3:
        continue
    swap_identity = idx + 1
    # Path to training images
    img_dirA = "../faces/cacd_auth/" + str(idx)
    img_dirA_bm_eyes = '../faces/binary_masks-cacd/'+ str(idx) +'_eyes'
    img_dirB = "../faces/cacd_auth/" + str(swap_identity)
    img_dirB_bm_eyes = '../faces/binary_masks-cacd/'+ str(swap_identity) +'_eyes'

    if (not os.path.exists(img_dirA)) or (not os.path.exists(img_dirB)):
        continue

    assert os.path.exists(img_dirA), "Dir A does not exist"
    assert os.path.exists(img_dirB), "Dir B does not exist"

    print("working on id "+str(idx)+" and "+str(swap_identity))

    # Path to saved model weights
    models_dir = "./models-cacd/AtoB_5_1st_half/id_"+str(idx)+"_"+str(swap_identity)
    ...
    # Get filenames
    train_A_all = glob.glob(img_dirA+"/*.*")

    ages = []
    for file in train_A_all:
        age = os.path.split(file)[-1].split('.')[0].split('_')[0]
        if age not in ages:
            ages.append(age)
    random.seed(100)
    random.shuffle(ages)

    num_ages_for_training = len(ages)//2 # we will take only second 2nd half
    selected_ages = ages[:num_ages_for_training]  # 1st half for learning generator
    ...
    def reset_session(save_path):
        ...
        train_batchA = DataLoader(train_A, train_AnB, batchSize, img_dirA_bm_eyes,
                                  RESOLUTION, num_cpus, K.get_session(), **da_config, inference_mode=False)
    ...
    try:
        start_iter
        print(f"Resume training from iter {start_iter}.")
    except:
        start_iter = 0
    ...
    with tf.device('/gpu:0'):
        ...
        train_batchB = DataLoader(train_B, train_AnB, batchSize, img_dirB_bm_eyes,
                                  RESOLUTION, num_cpus, K.get_session(), **da_config, inference_mode=False)
    for gen_iterations in range(start_iter, TOTAL_ITERS):
        ...
        if gen_iterations % display_iters == 0:
            ...
            showG(tA, tB, model.path_A, model.path_B, batchSize, save=True, save_path=models_dir,
                  filename="final_result.jpg")
            #print("Masks:")
            showG_mask(tA, tB, model.path_mask_A, model.path_mask_B, batchSize, save=True, save_path=models_dir)
            #print("Reconstruction results:")
            showG(wA, wB, model.path_bgr_A, model.path_bgr_B, batchSize, save=True, save_path=models_dir,
                  filename="recons_result.jpg")
            errGA_sum = errGB_sum = errDA_sum = errDB_sum = 0
            for k in ['ttl', 'adv', 'recon', 'edge', 'pl']:
                errGAs[k] = 0
                errGBs[k] = 0
...
# conda activate tf-gpu-cuda8 (cpu only)
# conda activate faceswap (gpu)
